...
class Verisure:
    ...
    async def login(self, mfa: bool, username, password, cookieFileName='~/.verisure_mfa_cookie'):
        ...
        if mfa:
            ...
            for url in urls:
                out = await self._doSession(method="POST", url=url, headers=self.headers, auth=self.auth)
            ...
            try:
                ...
                try:
                        out = await self._doSession(method="POST", url=urls[1], headers=self.headers, auth=self.auth)
                        print("login except ")
                        print(urls[1])
                        if 'errors' not in out:
                            await self.getAllInstallations()
    ...
    async def getMfaToken(self, username, password, cookieFileName='~/.verisure_mfa_cookie'):
        ...
        await self._doSession(method="POST", url="https://m-api01.verisure.com/auth/login", headers=self.headers, auth=self.auth)
                
        # Step 2: call  auth/mfa and Verisure vill send you a SMS with a code valid for 300 seconds i.e 5 minutes
        await self._doSession(method="POST", url="https://m-api01.verisure.com/auth/mfa", headers=self.headers)
        ...
        await self._doSession(method="POST", url="https://m-api01.verisure.com/auth/mfa/validate", headers=self.headers, data=json.dumps(tok))
            
        # session.cookies contains stepUpCookie, vid, vs-access and vs-refresh

        # Step 4:  call auth/trust and get the trust token
        await self._doSession(method="POST", url="https://m-api01.verisure.com/auth/trust", headers=self.headers)
    ...
    async def renewToken(self):
        ...
        try:
            await self._doSession(method="POST", url=urls[0], headers=self.headers)
        except:
            try:
                await self._doSession(method="POST", url=urls[1], headers=self.headers)
    ...
    async def logout(self):
        ...
        try:
            await self._doSession(method="DELETE", url=urls[0], headers=self.headers)
            await self.session.close()
        except:
            try:
                await self._doSession(method="DELETE", url=urls[1], headers=self.headers)
                await self.session.close()
    ...
    async def _doRequest(self, body):
        ...
        try:
            out = await self._doSession(method="POST", url=urls[0], headers=self.headers, data=json.dumps(list(body)))
            if 'errors' in out:
                out2 = await self._doSession(method="POST", url=urls[1], headers=self.headers, data=json.dumps(list(body)))
                if 'errors' in out2:
                    return {}
                else:
                    return out2
    ...
    async def getAllInstallations(self):
        ...
        response = await self._doRequest(body)
